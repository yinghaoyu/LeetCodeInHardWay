...
// 什么是 离线 ？
// 对于一道题目会给出若干询问，而这些询问是全部提前给出的，也就是说，你不必按照询问的顺序依次对它们进行处理，而是可以按照某种顺序（例如全序、偏序（拓扑序）、树的 DFS 序等）或者把所有询问看成一个整体（例如整体二分、莫队算法等）进行处理。
...
class Solution
{
 public:
  ...
  {
    // 将边按权值从小到大排序
    std::sort(edgeList.begin(), edgeList.end(), [](vector<int> &l, vector<int> r) { return l[2] < r[2]; });
    vector<int> idxs(queries.size());
    // 从0开始分配递增索引
    std::iota(idxs.begin(), idxs.end(), 0);
    // 将查询按照边权限制从小到大排序
    std::sort(idxs.begin(), idxs.end(), [&](int i, int j) { return queries[i][2] < queries[j][2]; });
    UnionFind uf(n);
    int i = 0;
