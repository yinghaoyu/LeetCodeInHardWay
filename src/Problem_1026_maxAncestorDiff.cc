...
class Solution
{
 public:
   // mi 为父节点的最小值
   // ma 为父节点的最大值
  int process(TreeNode *cur, int mi, int ma)
  {
    if (cur == nullptr)
    {
      return 0;
    ...
    diff = std::max(diff, process(cur->left, mi, ma));
    diff = std::max(diff, process(cur->right, mi, ma));
    return diff;
  }

  int maxAncestorDiff(TreeNode *root) { return process(root, root->val, root->val); }
};
